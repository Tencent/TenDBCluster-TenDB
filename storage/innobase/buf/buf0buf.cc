...
bool buf_chunk_t::madvise_dump() {
  ...
  /* This could always be true, if it is previously built on a machine whose glibc supports MADV_DODUMP */
  if (madvise(mem, mem_size(), MADV_DODUMP)) {
    ib::warn() << "Disabling @@core_file because @@innodb_buffer_pool_in_core_file is disabled, yet madvise(" << mem << "," << mem_size() << "," << " MADV_DONTDUMP failed with " << strerror(errno);
    return false;
  }
  return true;
...
#ifdef HAVE_MADV_DONTDUMP
  /* This could always be true, if it is previously built on a machine whose glibc supports MADV_DONTDUMP */
